...
def md5_check_single(file_path, file_md5):
    '''
...
def md5_check_remove(file_path_list, md5_list):
    """
    Check the MD5 checksum of the files using joblib
    If the MD5 is incorrect, the file will be removed.

    Args:
        file_path_list (list): A list of file paths
        md5_list (list): A list of MD5 checksums
    """
    from joblib import Parallel, delayed
    Parallel(n_jobs=-1)(delayed(md5_check_single)(file_path, file_md5) for file_path, file_md5 in zip(file_path_list, md5_list))
...
if __name__ == '__main__':
    ...
    # If argv = "--Check-md5-only", then only check the MD5 checksum
    if len(sys.argv) > 1:
        if '--check-md5-only' in sys.argv:
            print('Only check the MD5 checksum.')
            # file_path format: "root_path/Model/Experiment/Variable/..."
            file_path_list = [os.path.join(root_path,
                file_list['Model'][i],
                file_list['Experiment'][i],
                file_list['Variable'][i],
                file_list['fileUrl'][i].split('/')[-1]) for i in range(len(file_list))]
            md5_list = file_list['fileMD5'].tolist()
            md5_check_remove(file_path_list, md5_list)
            sys.exit()
